...
This artifact is self-contained, and supports full reproduction of the paper’s results. It includes all the code and scripts to generate the static and dynamic results reported in the paper, including graphs and every numerical result that has been discussed. All the intermediate data for data collection as well as the data collection script has also been included and every intermediate analysis result has also been included. The analyses have been made configurable so that they can be run on any given NPM package which supports the reuse of the analyses. Overall, we believe the artifact meets the criteria for Evaluated – Reusable: it is well-structured, carefully documented, complete with respect to the paper’s core results, and straightforward to exercise and extend beyond the original study.